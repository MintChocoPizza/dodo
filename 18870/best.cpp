...
struct Info { int val, idx; };

struct RadixTraitsInfo {
    static const int nBytes = sizeof(int);
    static const int kMSB = 0x80 << (sizeof(int) - 1) * 8;
    int kth_byte(const Info& x, int k) {
        return (x.val ^ kMSB) >> kx::kRadixBits * k & kx::kRadixMask;
    }
    bool compare(const Info& x, const Info& y) { return x.val < y.val; }
};

int main() {
    fastio;
    int n; cin >> n; Info v[n];
	for (int i = 0, t; i < n; i++) cin >> t, v[i] = { t, i };
	kx::radix_sort(v, v + n, RadixTraitsInfo());

	vector<int> ans(n);
	for (int i = 0, t = -1; i < n; i++) {
		if (!i || v[i - 1].val ^ v[i].val) t++;
		ans[v[i].idx] = t;
	}
